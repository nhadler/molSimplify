...
def test_TESTNAME(tmp_path):
    testName="TESTNAME"
    threshMLBL = 0.1
    threshLG =  1.0
    threshOG = 2.0
    [passNumAtoms,passMLBL,passLG,passOG,pass_report,pass_qcin] = hp.runtest(tmp_path,testName,threshMLBL,threshLG,threshOG)
    assert passNumAtoms
    assert passMLBL
    assert passLG
    assert passOG
...
def writeNoffTest(name):
    ...
    content = '''
def test_TESTNAME_No_FF(tmp_path):
    testName="TESTNAME"
    threshMLBL = 0.1
    threshLG =  1.0
    threshOG = 2.0
    [passNumAtoms,passMLBL,passLG,passOG,pass_report,pass_qcin] = hp.runtestNoFF(tmp_path,testName,threshMLBL,threshLG,threshOG)
    assert passMLBL
    assert passLG
    assert passOG
    assert pass_report
